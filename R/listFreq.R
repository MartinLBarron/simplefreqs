...
#' @param sortResults If TRUE (default), sort output in descending order of n.
#'   If FALSE, sort output in ascending order of levels
#' @param levelWarning if TRUE (default) gives an error if the variable passed
...
listFreq <- function(df,..., saveResults=F, printResults=T, sortResults=T, levelWarning=T, na.rm=F){
  ...
  if (levelWarning==T){
    ...
    l <-nrow(dftemp)
    if (l>50){
      stop(paste("Variables contains more than 50 levels (has", l, "levels). Set levelWarning=FALSE to proceed."))
    }
  }
...
# check <- listFreq(dfgss, year, race,marital, levelWarning=FALSE)
#
