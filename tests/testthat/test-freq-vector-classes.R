...
test_that("Works with Character", {
  expect_snapshot(freq(as.character(iris$Species), plot = F, markdown=F))
})
...
test_that("Works with character2", {
  ...
  df <- data.frame(x=x)
  expect_snapshot(freq(df, x, plot = F, markdown=F))
})


test_that("Works with Numeric", {
  expect_snapshot(freq(iris, Sepal.Width, plot = F, markdown=F))
})
...
test_that("Works with Dates", {
  ...
  expect_snapshot(freq(df$dt, plot = F, markdown=F))
})
...
test_that("Works with Times", {
  ...
  expect_snapshot(freq(df$time, plot = F, markdown=F))
})
...
test_that("Works with logical", {
  ...
  expect_snapshot(freq(x, plot = F, markdown=F))
})
